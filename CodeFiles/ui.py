...
class CityMapUI:
    ...
    def draw_sidebar(self):
        ...
        y = TOPBAR_H + 10
        #title block - Urban Intelligence System
        title_str = "Urban Intelligence System"
        title = self.font_heading.render(title_str, True, COL_GOLD_BRIGHT)
        cx = SIDEBAR_W // 2
    ...
    def _run_splash(self):
        splash_font_big = _load_font("Cinzel-Bold.ttf", 52)
        splash_font_sub = _load_font("Cinzel-Regular.ttf", 22)
        splash_font_btn = _load_font("Cinzel-SemiBold.ttf", 20)
        splash_font_tiny = _load_font("IMFellEnglish-Regular.ttf", 14)
        clock = pygame.time.Clock()
        # particle system
        particles = []
        for _ in range(120):
            particles.append({
                'x': _rand_mod.uniform(0, SCREEN_WIDTH),
                'y': _rand_mod.uniform(0, SCREEN_HEIGHT),
                'r': _rand_mod.uniform(1, 3),
                'sp': _rand_mod.uniform(0.15, 0.6),
                'a': _rand_mod.randint(40, 160),
                'dx': _rand_mod.uniform(-0.3, 0.3),
            })
        # orbiting dots
        orbs = [{'angle': i * (2 * math.pi / 6), 'dist': 140 + i * 8, 'speed': 0.008 + i * 0.002, 'size': 3 + i % 3} for i in range(6)]
        frame = 0
        btn_hover = False
        fade_alpha = 255  # start fully black, fade in
        fade_out = False
        fade_out_alpha = 0
        while True:
            frame += 1
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    if btn_rect.collidepoint(mx, my) and not fade_out:
                        fade_out = True
                if event.type == pygame.KEYDOWN:
                    if event.key in (pygame.K_RETURN, pygame.K_SPACE) and not fade_out:
                        fade_out = True
            # fade in
            ...
            # fade out
            if fade_out:
                fade_out_alpha = min(255, fade_out_alpha + 6)
                if fade_out_alpha >= 255:
                    return  # proceed to main
            self.screen.fill((5, 7, 12))
            # draw particles
            for p in particles:
                p['y'] -= p['sp']
                p['x'] += p['dx'] + math.sin(t + p['y'] * 0.01) * 0.2
                if p['y'] < -5:
                    p['y'] = SCREEN_HEIGHT + 5
                    p['x'] = _rand_mod.uniform(0, SCREEN_WIDTH)
                alpha = int(p['a'] * (0.5 + 0.5 * math.sin(t * 2 + p['x'])))
                alpha = max(20, min(200, alpha))
                ps = pygame.Surface((int(p['r']*2), int(p['r']*2)), pygame.SRCALPHA)
                pygame.draw.circle(ps, (180, 140, 60, alpha), (int(p['r']), int(p['r'])), int(p['r']))
                self.screen.blit(ps, (int(p['x'] - p['r']), int(p['y'] - p['r'])))
            cx, cy = SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 40
            # orbiting dots
            for orb in orbs:
                orb['angle'] += orb['speed']
                ox = cx + math.cos(orb['angle']) * orb['dist']
                oy = cy + math.sin(orb['angle']) * orb['dist'] * 0.45
                oa = int(100 + 80 * math.sin(t * 3 + orb['angle']))
                os_surf = pygame.Surface((orb['size']*2, orb['size']*2), pygame.SRCALPHA)
                pygame.draw.circle(os_surf, (200, 160, 70, oa), (orb['size'], orb['size']), orb['size'])
                self.screen.blit(os_surf, (int(ox - orb['size']), int(oy - orb['size'])))
            # decorative ring
            ring_r = 160 + int(8 * math.sin(t * 0.8))
            ring_surf = pygame.Surface((ring_r*2+4, ring_r*2+4), pygame.SRCALPHA)
            pygame.draw.circle(ring_surf, (120, 90, 35, 60), (ring_r+2, ring_r+2), ring_r, 2)
            self.screen.blit(ring_surf, (cx - ring_r - 2, cy - ring_r - 2))
            ring_r2 = 180 + int(6 * math.sin(t * 0.6 + 1))
            ring_surf2 = pygame.Surface((ring_r2*2+4, ring_r2*2+4), pygame.SRCALPHA)
            pygame.draw.circle(ring_surf2, (90, 68, 28, 35), (ring_r2+2, ring_r2+2), ring_r2, 1)
            self.screen.blit(ring_surf2, (cx - ring_r2 - 2, cy - ring_r2 - 2))
            # title glow
            glow_a = int(30 + 20 * math.sin(t * 1.5))
            glow_surf = pygame.Surface((500, 80), pygame.SRCALPHA)
            pygame.draw.ellipse(glow_surf, (180, 140, 55, glow_a), (0, 0, 500, 80))
            self.screen.blit(glow_surf, (cx - 250, cy - 55))
            # main title
            title_text = "LAHORE CITY EXPLORER"
            title_surf = splash_font_big.render(title_text, True, COL_GOLD_SHINE)
            self.screen.blit(title_surf, (cx - title_surf.get_width() // 2, cy - title_surf.get_height() // 2))
            # subtitle
            sub_text = "Urban Intelligence System"
            sub_surf = splash_font_sub.render(sub_text, True, COL_GOLD_DIM)
            self.screen.blit(sub_surf, (cx - sub_surf.get_width() // 2, cy + 40))
            # separator line
            line_w = int(200 + 40 * math.sin(t))
            line_a = int(80 + 40 * math.sin(t * 2))
            line_surf = pygame.Surface((line_w, 2), pygame.SRCALPHA)
            line_surf.fill((180, 138, 55, line_a))
            self.screen.blit(line_surf, (cx - line_w // 2, cy + 75))
            # decorative diamonds
            for dx_off in [-line_w//2 - 8, line_w//2 + 8]:
                d_cx = cx + dx_off
                d_cy = cy + 76
                pygame.draw.polygon(self.screen, COL_GOLD_DIM, [
                    (d_cx, d_cy - 4), (d_cx + 4, d_cy), (d_cx, d_cy + 4), (d_cx - 4, d_cy)
                ])
            # enter button
            btn_w, btn_h = 220, 50
            btn_x = cx - btn_w // 2
            ...
            if btn_hover:
                bg_c = (30, 25, 15)
                brd_c = COL_GOLD_BRIGHT
                txt_c = COL_GOLD_SHINE
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
            else:
                bg_c = (18, 16, 10)
                brd_c = (int(120 * pulse), int(90 * pulse), int(35 * pulse))
                txt_c = COL_GOLD
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            pygame.draw.rect(self.screen, bg_c, btn_rect, border_radius=8)
            pygame.draw.rect(self.screen, brd_c, btn_rect, 2, border_radius=8)
            # inner glow on hover
            if btn_hover:
                inner = pygame.Surface((btn_w, btn_h), pygame.SRCALPHA)
                pygame.draw.rect(inner, (180, 140, 55, 20), (0, 0, btn_w, btn_h), border_radius=8)
                self.screen.blit(inner, (btn_x, btn_y))
            btn_label = splash_font_btn.render("E N T E R", True, txt_c)
            self.screen.blit(btn_label, (btn_rect.centerx - btn_label.get_width()//2, btn_rect.centery - btn_label.get_height()//2))
            # bottom credits
            cred = splash_font_tiny.render("CityMind  •  Interactive Urban Map", True, COL_TEXT_GHOST)
            self.screen.blit(cred, (cx - cred.get_width()//2, SCREEN_HEIGHT - 40))
            # corner brackets
            bk_col = (90, 68, 28, 80)
            bk_len = 30
            for (bx, by, dx, dy) in [(20, 20, 1, 1), (SCREEN_WIDTH-20, 20, -1, 1), (20, SCREEN_HEIGHT-20, 1, -1), (SCREEN_WIDTH-20, SCREEN_HEIGHT-20, -1, -1)]:
                pygame.draw.line(self.screen, COL_BORDER_GOLD, (bx, by), (bx + bk_len*dx, by), 1)
                pygame.draw.line(self.screen, COL_BORDER_GOLD, (bx, by), (bx, by + bk_len*dy), 1)
            # fade overlay
            if fade_alpha > 0:
                fo = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                fo.fill((0, 0, 0, fade_alpha))
                self.screen.blit(fo, (0, 0))
            if fade_out_alpha > 0:
                fo = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
                fo.fill((0, 0, 0, fade_out_alpha))
                self.screen.blit(fo, (0, 0))
